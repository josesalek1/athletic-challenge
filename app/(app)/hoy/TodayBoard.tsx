...
export default function TodayBoard({
  ...
  const [habits, setHabits] = useState(challenges);
  const [habitSection, setHabitSection] = useState(challenges[0]?.id ?? '');
  ...
  const line = dayLine(dateLabel, results);
  ...
  async function deleteHabit(habit: Challenge) {
    ...
    setHabitFeedback('Activity deleted.');
  }

  function habitActions(habit: Challenge) {
    return (
      <div className="habit-card-actions">
        {habit.kind !== 'checklist' && <Link className="btn btn-ghost" href={`/hoy/habits/${habit.id}`}>Edit activity</Link>}
        <button className="btn-ghost" disabled={busy} onClick={() => deleteHabit(habit)}>Delete activity</button>
      </div>
    );
  }
    ...
    <main className="wrap">
      ...
      <section className="today-section habit-section" aria-labelledby="my-habits-heading">
        <div className="between today-section-heading">
          <p className="eyebrow today-section-label" id="my-habits-heading">My activities</p>
          <Link className="btn btn-water new-habit-link" href="/hoy/habits/new">+ New activity</Link>
        </div>

        {habits.length === 0 ? (
          <p className="empty">Create an activity to start tracking your day.</p>
        ) : (
          <>
            <div className="chips" role="tablist" aria-label="My activities">
              {habits.map((habit) => (
        ...
        {habitFeedback && (
          <p className="settings-feedback" data-tone={habitFeedback === 'Activity deleted.' ? 'success' : 'error'}>
            {habitFeedback}
          </p>
        )}
      </section>
      <section className="card" aria-label="WhatsApp report">
        <div className="between" style={{ marginBottom: 10 }}>
          <p className="eyebrow">Daily report</p>
          <p className="num" style={{ fontSize: 12, color: 'var(--mist)' }}>{results.length}/{habits.length}</p>
        </div>
        <p className="num" style={{ fontSize: 14, lineHeight: 1.6, marginBottom: 14 }}>{line}</p>
        <a className="btn btn-water" style={{ width: '100%' }} href={waLink(line)}
           target="_blank" rel="noopener noreferrer">Open in WhatsApp</a>
        <p className="muted" style={{ marginTop: 10 }}>Choose the group and send the message yourself.</p>
      </section>
    </main>
  );
}
